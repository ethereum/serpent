...
Node subst(Node p,
           ...
           std::string varflag,
           Metadata metadata) {
    if (p.type == TOKEN && p.val[0] == '@') {
        if (dict.count(p.val.substr(1))) return dict[p.val.substr(1)];
        else return token(varflag + p.val.substr(1), metadata);
    }
        ...
        for (int i = 0; i < p.args.size(); i++) {
            args.push_back(subst(p.args[i], dict, varflag, metadata));
        }
        return astnode(p.val, args, p.metadata);
    }
}

bool untouchable(Node node) {
    return node.val == "set" || node.val == "ref" || node.val == "get";
}


Node apply_rules(Node node) {
    int pos = 0;
    std::string prefix = "_temp"+intToDecimal(counter)+"_";
    counter += 1;
    while(1) {
